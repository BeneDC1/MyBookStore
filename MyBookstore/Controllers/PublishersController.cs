    ...
    public class PublishersController : Controller
    {
                    ...
                    using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                    {
                        using (DataTable dt = new DataTable())
                        {
                            ...
                            foreach (DataRow row in dt.Rows)
                            {
                                ...
                                publishers.Name = row["pubName"].ToString();                              
                                list.Add(publishers);
                            }
                        }
                    }
        ...
        public ActionResult Create(PublishersModels publishers)
        {
            ...
            using (SqlConnection con = new SqlConnection(Helper.GetCon()))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    cmd.Parameters.AddWithValue("@authorLN", publishers.Name);                
                    cmd.ExecuteNonQuery();
                    return RedirectToAction("Index");
                }
            }
        ...
        public ActionResult Edit(PublishersModels publishers)
        {
            using (SqlConnection con = new SqlConnection(Helper.GetCon()))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    cmd.Parameters.AddWithValue("@pubName", publishers.Name);
